...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Main message handler - routes based on current state"""
    ...
    if not context.user_data.get("service"):
        # No service selected yet - expect service selection
        await select_service(update, context, text)
    else:
        # Service already selected - expect phone number
        await check_number_and_call_api(update, context)


async def select_service(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
